# TPUSH/TPOP Support in a5 Simulation

This document summarizes the CPU_SIM context plumbing used by TPUSH/TPOP in
`a5sim`, including the host-side follow-up that replaced `thread_local`
...
1. Add a host-side CPU_SIM context module in `src/a5/platform/sim/host/cpu_sim_context.cpp`
...
- Host CPU_SIM context
  - `src/a5/platform/sim/host/cpu_sim_context.cpp`
...
- Platform hook glue
  - `src/a5/platform/include/aicpu/cpu_sim_task_cookie.h`
...
The design was validated with:

- `examples/a5/tensormap_and_ringbuffer/bgemm/`

This case exercises the real mixed-core path:

- AIC: `TPUSH`
- AIV: `TPOP`

The host-side `cpu_sim_context.cpp` implementation was also independently
checked after the pthread conversion with direct C++ compilation.
...
`a5sim`. It does not describe every remaining TLS-like variable under
`src/a5/platform/sim/`.
